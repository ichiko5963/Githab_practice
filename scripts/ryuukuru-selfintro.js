...
const SLACK_SELFINTRO_CHANNEL_ID = process.env.SLACK_SELFINTRO_CHANNEL_ID || 'C09B8K9KF43'; // デフォルトをチャンネルIDに設定
...
async function main() {
  try {
    ...
    // チャンネルIDを直接使用（環境変数またはデフォルト）
    const channelId = SLACK_SELFINTRO_CHANNEL_ID;
    console.log(`🎯 対象チャンネルID: ${channelId}`);
    
    // チャンネルが存在するか確認
    try {
      const channelInfo = await slack.conversations.info({ channel: channelId });
      console.log(`✅ チャンネル確認: ${channelInfo.channel.name} (${channelInfo.channel.id})`);
    } catch (error) {
      console.error(`❌ チャンネルアクセスエラー: ${error.message}`);
      throw new Error(`チャンネルにアクセスできません: ${channelId}`);
    }
    ...
    // メッセージを取得（4日間の全てのメッセージ）
    console.log(`🔍 ${channelId} チャンネルの4日間のメッセージを全て取得します`);
    const messages = await getMessagesInPeriod(channelId, startDate, endDate);
    
    // 自己紹介メッセージをフィルタリング（超緩和版：ほぼ全て含める）
    const introMessages = messages.filter(msg => {
      ...
      // ほぼ全てのメッセージを含める（3文字以上は全て含める）
      return msg.text.length >= 3;
    });
