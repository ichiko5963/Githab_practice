...
const channelId = process.env.SLACK_AIRCLE_CHANNEL_ID; // #aircle-人材交流部
const timezone = process.env.TZ || 'Asia/Tokyo';

if (!slackToken || !channelId) {
  console.error('Missing SLACK_BOT_TOKEN or SLACK_AIRCLE_CHANNEL_ID');
  process.exit(1);
}
...
const RYUUKURU_CONFIG = {
  ...
  pronoun: 'オイラ',
  catchphrase: 'リュウクル参上！！',
  ending: ['リュークル応援してるぞ！', 'ご視聴ありがとうございました！']
};

// メンション対象のユーザーID（実際のSlackのユーザーIDに置き換える必要があります）
const MENTION_USERS = {
  miori: 'U09C8GTJGDD' // @みおり（運営）の実際のユーザーID
};

// スプレッドシートのリンク
const SPREADSHEET_URL = 'https://docs.google.com/spreadsheets/d/1ZP9Of73A1FXZ1-EVKdeVHzV56MN7RWrd3bsNWyL-4zE/edit?usp=sharing';

/**
 * 現在の曜日を取得
 * @returns {string} 曜日（'土' または '月'）
 */
function getCurrentDayOfWeek() {
  const now = utcToZonedTime(new Date(), timezone);
  const dayOfWeek = now.getDay();
  return dayOfWeek === 6 ? '土' : dayOfWeek === 1 ? '月' : null;
}
 ...
 */
function generateExchangeMeetingReminderMessage() {
  const dayOfWeek = getCurrentDayOfWeek();
  
  return `${RYUUKURU_CONFIG.catchphrase}

今日は週の${dayOfWeek}曜日、19時のリマインドタイムだぞ！
交流会の準備はちゃんと進んでるか？
...
<@${MENTION_USERS.miori}>、必ずチェックしてタスクを振ってくれ！
そして他のメンバーに1on1の日程調整を任せて、誰がやるか決めろ！

仲間が増えるのは最高に嬉しいことだ！
${RYUUKURU_CONFIG.ending[0]}
${RYUUKURU_CONFIG.ending[1]}`;
}
 ...
 */
async function main() {
  console.log('🚀 リュークル人材交流部リマインドを開始します...');
  ...
  const dayOfWeek = getCurrentDayOfWeek();
  if (!dayOfWeek) {
    console.log('📅 今日は土曜日でも月曜日でもないため、リマインドをスキップします');
    return;
  }
  
  console.log(`📅 今日は${dayOfWeek}曜日です - リマインドを実行します`);
  
  // メッセージを生成して投稿
  const message = generateExchangeMeetingReminderMessage();
  console.log('📝 交流会リマインドメッセージを生成しました');
  
  // Slackに投稿
  await postReminderMessage(message);
  
  console.log('✅ リュークル人材交流部リマインドが完了しました');
}
